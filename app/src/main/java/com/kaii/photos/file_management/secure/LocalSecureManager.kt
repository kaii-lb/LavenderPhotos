...
class LocalSecureManager @Inject constructor(
    ...
    ): Result<FileOperationItemMetadata, FileOperationError> = withContext(Dispatchers.IO) {
    ...
    override suspend fun restore(
        ...
        if (iv == null && secureDao.getIvFromSecuredPath(fileToBeRestored.absolutePath) != null) {
            Log.e(LocalSecureManager::class.qualifiedName, "Refusing to restore ${media.item.displayName}: iv unrecoverable, keeping it secured")
        ...
        try {
            contentResolver.copyUriToUri(originalUri, newUri)
