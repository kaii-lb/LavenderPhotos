...
import io.github.kaii_lb.lavender.snackbars.LavenderSnackbarEvent.MessageEvent
import kotlinx.coroutines.launch
import kotlin.io.encoding.Base64
import kotlin.uuid.ExperimentalUuidApi
import kotlin.uuid.Uuid

// TODO: rearchitect this whole function and clean it up
...
fun AlbumAddChoiceDialog(
    ...
    LavenderDialogBase(
        ...
        Column(
            ...
            val safActivityLauncher = createPersistablePermissionLauncher(
                onGranted = { uri ->
                    PhotosApplication.appModule.scope.launch {
                        try {
                            ...
                            val info = context.packageManager
                                ...
                                .applicationInfo?.loadLabel(context.packageManager) ?: packageName
                            ...
                            addAlbum(
                                AlbumType.SAFFolder(
                                    ...
                                    pinned = false,
                                    immichId = null,
                                    path = "$info > $folderName",
                                    base64TreeUri = Base64.encode(uri.toString().encodeToByteArray())
                                )
                            )
                        } catch (e: PackageManager.NameNotFoundException) {
                            Log.e(
                                "com.kaii.photos.compose.dialogs.AlbumAddChoiceDialog.safActivityLauncher",
                                "Failed to add SAF folder!",
                                e
                            )
                        }
                    }
            ...
            PreferencesRow(
                title = stringResource(id = R.string.albums_network_share),
                summary = stringResource(id = R.string.albums_network_share_desc),
                position = RowPosition.Bottom,
                iconResID = R.drawable.smb_share,
                enabled = NetworkShareCapabilityProvider.hasNetworkCapabilities
            ) {
                safActivityLauncher.launch(null)
            }
        }
